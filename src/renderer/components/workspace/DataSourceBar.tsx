    ...
    >
      ...
      >
        <button
          className="btn sm"
          style={{
            background: sourceMode === 'rosbag' ? 'var(--color-bg-3)' : 'transparent',
      ...
      {filename && (
        <div className="row gap-2">
          <Icon.File size={13} />
          <span className="mono" style={{ fontSize: 12, color: 'var(--color-text-1)' }}>
            {filename}
          </span>
          {meta && (
            <span className="dim mono" style={{ fontSize: 11 }}>
              {meta.totalMessages.toLocaleString()} msgs · {meta.topicsCount} topics
            </span>
          )}
        </div>
      )}
      ...
      <div className="row gap-1" style={{ marginLeft: 8 }}>
        ...
        <button className="btn ghost icon sm" title="Share (not available)" disabled>
          <Icon.Share size={13} />
        </button>
