    ...
    return (
      <div className="flex1 col" style={{ minWidth: 0, minHeight: 0 }}>
        ...
        >
          ...
          <div className="col gap-1" style={{ alignItems: 'center', textAlign: 'center' }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-text-0)' }}>
              {status === 'creating' ? 'Creating analysis session…' : 'AI is indexing topic channels…'}
            </span>
          ...
          >
            {Array.from({ length: 3 }, (_, i) => (
              <div key={i} className="row gap-4" style={{ padding: '8px 0' }}>
                <div
                  className="pulse"
                  style={{ width: 80, height: 12, background: 'var(--color-bg-3)', borderRadius: 4 }}
                />
                <div
                  className="flex1 pulse"
                  style={{ height: 12, background: 'var(--color-bg-2)', borderRadius: 4, opacity: 0.7 - i * 0.15 }}
                />
              </div>
            ))}
          </div>
