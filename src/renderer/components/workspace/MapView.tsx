...
const NAV_TOPIC_PATTERNS = ['/odom', '/tf', '/map', '/costmap', '/amcl', '/move_base', '/nav', '/path', '/cmd_vel']
...
export function MapView(): JSX.Element {
  ...
  const navTopics = topics.filter((t) =>
    NAV_TOPIC_PATTERNS.some((p) => t.name.toLowerCase().includes(p.toLowerCase()))
  )
  ...
  if (!sessionId) {
    return (
      <div className="col flex1" style={{ minHeight: 0 }}>
        ...
        <div
          className="flex1 col"
          style={{ alignItems: 'center', justifyContent: 'center', gap: 12, color: 'var(--color-text-3)' }}
        >
  ...
  return (
    <div className="col flex1" style={{ minHeight: 0 }}>
      ...
      >
        <span className="section-h">Map · trajectory</span>
        {navTopics.length > 0 && (
          <span className="pill sm ghost mono">{navTopics.length} nav topics</span>
        )}
        <div className="flex1" />
        <button className="btn ghost icon sm" title="Zoom (not available)" disabled>
          <Icon.Zoom size={13} />
        </button>
      </div>
        ...
        >
          ...
          <div className="col gap-2">
            ...
            <span style={{ fontSize: 12, color: 'var(--color-text-3)', lineHeight: 1.5 }}>
              2D trajectory visualization with costmap overlay, odometry path, and anomaly markers will
              be rendered from your bag's navigation topics.
            </span>
          </div>
          ...
          {navTopics.length > 0 ? (
            <div className="col" style={{ width: '100%', gap: 6 }}>
              <span style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--color-text-3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                Navigation topics found in this bag
              </span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center' }}>
                {navTopics.map((t) => (
                  <span key={t.name} className="pill sm ghost mono" style={{ fontSize: 10.5 }}>
                    {t.name}
                  </span>
                ))}
              </div>
