...
function AgentCard({
  ...
  const Comp = Icon[a.iconName] as ((props: { size?: number }) => JSX.Element) | undefined
  return (
    <div
      className="card"
      style={{ padding: 16, opacity: a.enabled ? 1 : 0.7, transition: 'opacity 0.15s' }}
    >
      <div className="row gap-3" style={{ marginBottom: 10 }}>
        ...
        <Toggle on={a.enabled} onChange={(v) => onToggle(a.id, v)} label={`${a.name} enabled`} />
      </div>
...
function MCPRow({
  ...
  return (
    <div
      className="row gap-3"
      style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-border-1)' }}
    >
      <div
        style={{
          width: 32,
      ...
      <div className="flex1" style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11.5, color: 'var(--color-text-2)', marginTop: 4 }}>{m.desc}</div>
      </div>
  ...
  return (
    <div className="card" style={{ margin: '0 0 18px', padding: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <FieldSelect label="Transport" options={['STDIO', 'HTTP', 'WebSocket']} />
        ...
        <div style={{ gridColumn: '1 / 3' }}>
          <FieldInput label="Authentication" placeholder="Bearer / Header / None" />
        </div>
      </div>
...
function FieldSelect({ label, options }: { label: string; options: string[] }): JSX.Element {
  return (
    <div className="col" style={{ gap: 6 }}>
      <label style={{ fontSize: 11.5, fontWeight: 500, color: 'var(--color-text-2)' }}>
        {label}
...
const ROUTING_OPTIONS = [
  { key: 'auto',     label: 'Auto-route', desc: 'Coordinator picks the best agent per turn' },
  { key: 'parallel', label: 'Parallel',   desc: 'All agents run, results merged & ranked' },
  { key: 'manual',   label: 'Manual',     desc: 'You pick the agent in each message' },
] as const
...
export function Agents(): JSX.Element {
  ...
  const handleAgentToggle = (id: string, v: boolean) =>
    setAgents((prev) => prev.map((a) => (a.id === id ? { ...a, enabled: v } : a)))

  const handleMCPToggle = (id: string, v: boolean) =>
    setMcps((prev) =>
      prev.map((m) => (m.id === id ? { ...m, status: v ? 'connected' : 'disabled' } : m)),
    )
        ...
        <div className="col">
          <div className="row gap-2">
            ...
            <span className="pill sm ghost mono">
              {enabledAgents}/{agents.length} agents · {connectedMcps}/{mcps.length} MCP
            </span>
          </div>
          <span className="dim" style={{ fontSize: 11.5, marginTop: 2 }}>
            Compose your copilot — toggle specialist agents and external tool servers.
          </span>
        </div>
        <div className="flex1" />
        {/* Segment control */}
        ...
        {section === 'agents' && (
          <div style={{ padding: 18 }}>
            ...
            <div className="card" style={{ marginTop: 22, padding: 16 }}>
              ...
              <div
                style={{ fontSize: 12, color: 'var(--color-text-2)', lineHeight: 1.5, marginBottom: 12 }}
              >
              ...
              <div className="row gap-2">
                {ROUTING_OPTIONS.map(({ key, label, desc }) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11, color: 'var(--color-text-2)' }}>{desc}</div>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 14 }}>
              <div className="row gap-2">
                ...
                <span style={{ fontSize: 12, color: 'var(--color-text-1)' }}>
                  MCP servers run locally on your machine. Outgoing tool calls show in{' '}
